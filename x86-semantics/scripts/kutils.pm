...
s/^\(#ifMInt TMP_BOOL_(\d+) #then mi\(1, 1\) #else mi\(1, 0\) #fi\)/(undefMInt)/g;
    ...
    for my $k ( keys %actual2psedoRegs ) {
        ...
        if ( $actual2psedoRegs{$k} eq "MemOff" ) {
            $replace_to = "MemOff";
        }
...
$semantics
            ...
            $ruleSemantics = qq(  <regstate> RSMap:Map </regstate>);
        }
...
module $module_name_uc
  ...
  $loadMemOffsetTemplate
  rule <k>
    $loadedValue execinstr ($enc:Opcode $type2_fill .Operands) =>
      $storeTemplate
  ...</k>
  $ruleSemantics
endmodule
  ...
  ...</k>
    ...
    ## The main Template for imm and reg.
    my $template = "";
    if ( "" eq $semantics ) {
